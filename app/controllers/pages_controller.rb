class PagesController < ApplicationController
  ...
  def cotizaciones
    ...
    @quotes = Quote.find_all_by_sending_details(email,:conditions=>["status like 'accepted' or status like 'requested' and (created_at > ? or ((request_date IS NOT NULL) and request_date > ?)) ",Time.zone.now - 3.months,Time.zone.now - 3.months])
    unless @quotes.empty?
